...
const textarea_caret_1 = require("textarea-caret");
...
class AppInbox extends HTMLElement {
    constructor() {
        ...
        for (var i = 0; i < 10; i++) {
            ...
            message.textContent =
                "Semper risus in hendrerit gravida rutrum quisque non tellus orci Dictum sit amet justo donec enim diam vulputate ut pharetra Sed viverra ipsum nunc aliquet bibendum enim facilisis gravida neque.";
...
class SmartText extends HTMLElement {
    ...
    load_chat(chat_string) {
        this.innerHTML = "";
        this.chat = parseChat(chat_string);
    ...
    add_line(line_string) {
        const line = parseLine(line_string);
        this.chat.push(line);
...
class SmartInput extends HTMLElement {
    constructor() {
        ...
        function input() {
            ...
            let word_start = before_selection.split(/(\w*\s*)*([^\s]*$)/gi).filter((string) => {
                return string !== "";
            })[0] || "";
            let word_end = after_selection.split(/(^[^\s]*)/i).filter((string) => {
                return string !== "";
            })[0] || "";
            let word = word_start + word_end;
            let matches = string_similarity_1.findBestMatch(word, keywords)
            ...
            let caret_pos = textarea_caret_1.default(input_elem, input_elem.selectionStart);
