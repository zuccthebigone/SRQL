...
import get_caret_pos from "textarea-caret";
...
class SmartInput extends HTMLElement {
	constructor() {
		...
		function input() {
			let before_selection = input_elem.value.substr(
				0,
				input_elem.selectionStart
			);
			let after_selection = input_elem.value.substr(input_elem.selectionEnd);

			let word_start =
				before_selection.split(/(\w*\s*)*([^\s]*$)/gi).filter((string) => {
					return string !== "";
				})[0] || "";
			let word_end =
				after_selection.split(/(^[^\s]*)/i).filter((string) => {
					return string !== "";
				})[0] || "";

			let word = word_start + word_end;

			let matches = findBestMatch(word, keywords)
