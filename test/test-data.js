...
const fs = require("fs");
const { ClientFactory } = require("../src/core/db");
...
async function delete_all_data() {
    client.query("DELETE FROM public.user; DELETE FROM srql; DELETE FROM srql_member");
}

async function insert_test_data() {

    const tables = await Promise.all(fs.readdirSync("./test/db/").map(get_table_data));
...
async function insert_row(table_name, row) {
    const value_string = Object.values(row)
        .map(data => `'${data}'`)
        .join(", ");
    await client.query(`INSERT INTO public.${table_name} VALUES (${value_string})`);
}
